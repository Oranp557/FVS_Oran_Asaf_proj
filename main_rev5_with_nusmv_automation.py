...
def run_nusmv_and_check(filename):
...
"""
    try:
        ...
        print("Full NuSMV output:\n", output)

        # Parse the result of check_ltlspec_bmc
        if "is true" in output.lower():
            print("\n✅ LTL Specification PASSED (is true).")
        elif "is false" in output.lower():
            print("\n❌ LTL Specification FAILED (is false).")
        else:
            print("\n⚠️ Could not determine LTL check result.")
...
if __name__ == "__main__":
    smv_file = "my_model.smv"  # Replace with your actual file
    run_nusmv_and_check(smv_file)
...
for board in boards:
    ...
    with open(("sokoban_boards"+str(i)+".smv"), "w") as f:
            ...
            f.write(smv_model)
    i=i+1
    resutls=run_nusmv_and_check("sokoban_boards"+str(i)+".smv")
    if resutls==1:
        print('winnable')
    else:
        print('not winnable')
